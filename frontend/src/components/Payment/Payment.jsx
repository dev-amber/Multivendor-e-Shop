...
const Payment = () => {
  ...
  const createOrder = (data, actions) => {
    return actions.order
      .create({
        ...
        application_context: {
          shipping_perference: "No Shipping",
        },
      })
      .then((orderId) => {
        return orderId;
  ...
  const onApprove = async (data, actions) => {
    return actions.order.capture().then(function (details) {
      ...
      let paymentInfo = payer; // this data is send to backend
  ...
  const paymentHandler = async (e) => {
    ...
    try {
      const config = {
        headers: {
          "Content-Type": "application/json", // config send req to backend
        },
      };
      ...
      const client_secret = data.client_secret; // cleient backend stripe
      ...
      if (result.error) {
        ...
        if (result.paymentIntent.status === "succeeded") {
          order.paymentInfo = {
            id: result.paymentIntent.id, // this is data like pass our model
